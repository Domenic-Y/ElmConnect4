    ...
    Place int ->
      ...
      if model.winner == Empty then
        if check4s newBoard then
            Debug.log("win")
            {model | board = newBoard, turnColour = Yellow, winner = model.turnColour}
        else
...
check4Across board = Array.foldl (||) False (Array.map (rowCheckFliped 0 Empty) board)

{-Checks if there a 4 pieces of the same colour in a vertical "row"-}
check4Down: Board -> Bool
check4Down board = (check4Across (rotateBoard board))
...
checkNegDiagonals  board =
  Debug.log(Debug.toString (rotateBoardNeg15 board))
...
getColumn board xInt = let yRange = Array.fromList <| List.range 0 6
                    in
                    Array.map (Maybe.withDefault Empty) (Array.map (Array.get xInt) (Array.map (Maybe.withDefault Array.empty) (Array.map (board |> Flip.flip Array.get) yRange)))

rotateBoardPos15: Board -> Board
rotateBoardPos15 board = let xRange = Array.fromList <| List.range 3 8
                    in
                    Array.map (getPosDiagonals board) xRange

rotateBoardNeg15: Board -> Board
rotateBoardNeg15 board = let xRange = Array.fromList <| List.range -3 2
                    in
                    Array.map (getNegDiagonals board) xRange


getPosDiagonals: Board -> Int -> Array.Array Colour
getPosDiagonals board int1 =
  let
    yRange = Array.fromList <| List.reverse <| List.range 0 int1
    xRange = Array.fromList <| List.range 0 int1
  in
  Array.map (Maybe.withDefault Empty) (Array.Extra.map2 (<|) (Array.map Array.get xRange) (Array.map (Maybe.withDefault Array.empty) (Array.map (board |> Flip.flip Array.get) yRange)))


getNegDiagonals: Board -> Int -> Array.Array Colour
getNegDiagonals board int1 =
  let
    yRange = Array.fromList <| List.range int1 8
...
rowCheck row count lastColour =
  if count >= 4 then
    ...
    if Array.isEmpty row then
      ...
      if ((Maybe.withDefault (Empty) (Array.get ((Array.length row) - 1) row)) == lastColour) && lastColour /= Empty then
        rowCheck (Array.slice 0 -1 row) (count + 1) (Maybe.withDefault (Empty) (Array.get ((Array.length row) - 1) row))
      else
        rowCheck (Array.slice 0 -1 row) (1) (Maybe.withDefault (Empty) (Array.get ((Array.length row) - 1) row))

{-places a piece-}
updateArray : Int -> Int -> Board -> Colour -> Board
updateArray colInt rowInt board colour =
  let
    unitPiece = Maybe.withDefault (Empty) (Array.get colInt (Maybe.withDefault (Array.empty) (Array.get rowInt board)))
    newRow = Array.set colInt colour (Maybe.withDefault (Array.empty) (Array.get rowInt board))
  in
...
viewBoard model = div [Att.class "parent"]
    ...
    , div [] (Array.toList(Array.map viewRow (model.board)))]
  ...
  case colour of
    Red ->
      div [] [Svg.svg [ Svg.Attributes.width "100", Svg.Attributes.height "100" ]
                      [Svg.circle [ Svg.Attributes.cx "50", Svg.Attributes.cy "50", Svg.Attributes.r "30", Svg.Attributes.fill "red" ] [ ]]]
    Yellow ->
      div [] [Svg.svg [ Svg.Attributes.width "100", Svg.Attributes.height "100" ]
                      [Svg.circle [ Svg.Attributes.cx "50", Svg.Attributes.cy "50", Svg.Attributes.r "30", Svg.Attributes.fill "yellow" ] [ ]]]
    Empty ->
      div [] [Svg.svg [ Svg.Attributes.width "100", Svg.Attributes.height "100" ]
                      [Svg.circle [ Svg.Attributes.cx "50", Svg.Attributes.cy "50", Svg.Attributes.r "30", Svg.Attributes.fill "gray" ] [ ]]]
